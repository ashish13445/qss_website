...
const renderChart = () => {
    ...
    new Chart(ctx, {
        type: 'bar',
        data: {
            labels: attendanceData.value.map(entry => entry.date), // Assuming your attendance data contains a 'date' property
            datasets: [{
                label: 'working hours',
...
const fetchUser = async () => {
 ...
 user.value = response.data;
  ...
  log_in_time.value = attendance.data[attendance.data.length]?attendance.data[attendance.data.length].clock_in: "";
  log_out_time.value = attendance.data[attendance.data.length]?attendance.data[attendance.data.length].clock_out:"";
}
onMounted(fetchUser);
    ...
    fetch(apiUrl)
        ...
        .then(data => {
            if (data.display_name) {
                ...
                // Now you can send the location name to your server along with other data
            }
        })
        .catch(error => {
        ...
        <div class="py-6">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <h3 class=" p-4  text-gray-400 font-extrabold text-2xl">Time & Attendance
                    </h3>
                    <div class="lg:flex p-4">
                        <div v-if="timerRunning" class="p-4 text-blue-800 font-bold mr-2 lg:w-1/4 my-auto">
                            <span class="text-blue-600 font-extrabold text-6xl">{{ formatTime(timer) }}</span>
                            <p class="text-small text-gray-400">Time spent</p>
                        </div>
                        <div v-else class="p-4 text-blue-800 font-bold mr-2 lg:w-1/4 my-auto">
                            <span class="text-blue-600 font-extrabold text-6xl">0:00:00</span>
                            <p class="text-small text-gray-400">Time spent</p>
                        </div>
                        <div class=" mr-4 lg:w-1/4 my-auto">
                            <div class="p-8 bg-blue-100 rounded font-bold">
                            <span class="text-blue-600 text-xl font-bold">{{ log_in_time }}</span>
                            <p class="text-small text-gray-400 ">Clock In Time</p>
                        </div>
                    </div>
                   
                        <div class="lg:flex lg:w-1/2 p-4 lg:ml-4">
                            <div class="bg-blue-500 mr-3 lg:block hidden">.</div>
